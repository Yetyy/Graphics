...
// Функция для создания текстурных координат для треугольника
void createTriangleTextureCoords(float* texture_coords, int offset) {
    texture_coords[offset * 2] = 0.0f;       // Левая вершина
    texture_coords[offset * 2 + 1] = 0.0f;

    texture_coords[(offset + 1) * 2] = 1.0f; // Правая вершина
    texture_coords[(offset + 1) * 2 + 1] = 0.0f;

    texture_coords[(offset + 2) * 2] = 0.5f; // Нижняя вершина
    texture_coords[(offset + 2) * 2 + 1] = 1.0f;
}
...
int main() {
    ...
    unsigned char* image_circles = stbi_load("ice_cream.jpg", &width1, &height1, &channels1, STBI_rgb);
    unsigned char* image_triangle = stbi_load("waffle.jpg", &width2, &height2, &channels2, STBI_rgb);

    if (!image_circles) {
        fprintf(stderr, "ERROR: could not load texture ice_cream.png\n");
        return 1;
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Рисование треугольника с текстурой вафли
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, texture_triangle);
        glUniform1i(use_triangle_texture_location, 1);
        glDrawArrays(GL_TRIANGLES, (NUM_SEGMENTS + 1) * 2, 3); // Треугольник

        // Рисование кругов с текстурой пломбира
